...
func main() {
	rootDir := "tmp/taxi/taxi-sha256" // Change to where to unzipped taxi-sha256.zip
	...
	for name, signature := range sigs {
		...
		go signWorker(fileName, signature, ch)
	}
	for range sigs {
		r := <-ch
		if r.err != nil {
...
// 我们可以指定channel的方向
func signWorker(fileName string, signature string, ch chan<- result) {
